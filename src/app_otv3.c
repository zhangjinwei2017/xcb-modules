...
static int otv3_exec(void *data, void *data2) {
	...
	for (i = 4; i < nfield - 6; i += 4) {
		...
		call   = !strcasecmp(fields[i + 1], "nan") || atof(fields[i + 1]) < 0.0
			? NAN : tri_amer_call(spot, strike, r, r, atof(fields[i + 1]), expiry, steps);
		put    = !strcasecmp(fields[i + 1], "nan") || atof(fields[i + 1]) < 0.0
			? NAN : tri_amer_put (spot, strike, r, r, atof(fields[i + 1]), expiry, steps);
		snprintf(res, sizeof res, "OTV3,%s.%03d,%s%sC%s%s|%f,%f,%f",
			...
			!strcasecmp(fields[i + 2], "nan") || atof(fields[i + 2]) < 0.0
				...
				: (fabs(atof(fields[i + 2]) - atof(fields[i + 1])) <= 0.000001
					? call
					: tri_amer_call(spot, strike, r, r, atof(fields[i + 2]),
						expiry, steps)),
			!strcasecmp(fields[i + 3], "nan") || atof(fields[i + 3]) < 0.0
				? NAN
				: (fabs(atof(fields[i + 3]) - atof(fields[i + 1])) <= 0.000001
					? call
					: tri_amer_call(spot, strike, r, r, atof(fields[i + 3]),
						expiry, steps)));
		out2rmp(res);
		snprintf(res, sizeof res, "OTV3,%s.%03d,%s%sP%s%s|%f,%f,%f",
			...
			!strcasecmp(fields[i + 2], "nan") || atof(fields[i + 2]) < 0.0
				...
				: (fabs(atof(fields[i + 2]) - atof(fields[i + 1])) <= 0.000001
					? put
					: tri_amer_put (spot, strike, r, r, atof(fields[i + 2]),
						expiry, steps)),
			!strcasecmp(fields[i + 3], "nan") || atof(fields[i + 3]) < 0.0
				? NAN
				: (fabs(atof(fields[i + 3]) - atof(fields[i + 1])) <= 0.000001
					? put
					: tri_amer_put (spot, strike, r, r, atof(fields[i + 3]),
						expiry, steps)));
		out2rmp(res);
	}
