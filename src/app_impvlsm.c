...
static char *fmt = "IMPVLSM,timestamp,contract,lastprice,vol,bidprice1,vol2,askprice1,vol3,spotprice";
...
static int impvlsm_exec(void *data, void *data2) {
	...
	if (p && p != contract && p != contract + dstr_length(contract) - 1 &&
		...
		if ((res = ALLOC(512))) {
			...
			snprintf(res, 512, "IMPVLSM,%s.%03d,%s|%.4f,%f,%.4f,%f,%.4f,%f,%.4f",
				datestr,
				quote->m_nMSec,
				contract,
				last,
				...
				spot);
			out2rmp(res);
			snprintf(res, 512, "IMPVLSM,%d,%d,%s,%.4f,%f,%.4f,%f,%.4f,%f,%.4f,%s,%s,%f,%f,%f,%d,"
				"%d,%d",
				quote->thyquote.m_nTime,
				quote->m_nMSec,
				contract,
