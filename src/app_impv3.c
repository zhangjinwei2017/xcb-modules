...
static char *fmt = "IMPV3,timestamp,contract,lastprice,vol,bidprice1,vol2,askprice1,vol3,spotprice";
...
static int impv3_exec(void *data, void *data2) {
	...
	if (p && p != contract && p != contract + dstr_length(contract) - 1 &&
		...
		if (fabs(last) <= 0.000001)
			...
			vol = impv_trinomial(spot, atof(strike), r, r, expiry, steps, last,
		...
		if (fabs(quote->thyquote.m_dMRJG1) <= 0.000001)
			...
			vol2 = impv_trinomial(spot, atof(strike), r, r, expiry, steps, quote->thyquote.m_dMRJG1,
				!strcasecmp(type, "C") ? AMER_CALL : AMER_PUT);
		/* FIXME: ask price 1 */
		if (fabs(quote->thyquote.m_dMCJG1) <= 0.000001)
			...
			vol3 = impv_trinomial(spot, atof(strike), r, r, expiry, steps, quote->thyquote.m_dMCJG1,
				!strcasecmp(type, "C") ? AMER_CALL : AMER_PUT);
		if ((res = ALLOC(512))) {
			...
			snprintf(res, 512, "IMPV3,%s.%03d,%s|%.2f,%f,%.2f,%f,%.2f,%f,%.2f",
				datestr,
				quote->m_nMSec,
				contract,
				last,
				...
				spot);
			out2rmp(res);
			snprintf(res, 512, "IMPV3,%d,%d,%s,%.2f,%f,%.2f,%f,%.2f,%f,%.2f,%s,%s,%s,%f,%f,%d,0,%d",
				quote->thyquote.m_nTime,
				quote->m_nMSec,
				contract,
				last,
