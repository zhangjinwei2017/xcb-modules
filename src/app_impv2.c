...
static char *fmt = "IMPV2,timestamp,contract,lastprice,vol,"
...
static int impv2_exec(void *data, void *data2) {
	...
	if (p && p != contract && p != contract + dstr_length(contract) - 1 &&
		...
		if (fabs(last) <= 0.000001)
			...
			vol = impv_bi(slast, strike, r, r, expiry, steps, last,
		...
		if (fabs(quote->thyquote.m_dMRJG1) <= 0.000001)
			...
			vol2 = impv_bi(slast, strike, r, r, expiry, steps, quote->thyquote.m_dMRJG1,
				!strcasecmp(type, "C") ? AMER_CALL : AMER_PUT);
		/* FIXME: ask price 1 */
		if (fabs(quote->thyquote.m_dMCJG1) <= 0.000001)
			...
			vol3 = impv_bi(slast, strike, r, r, expiry, steps, quote->thyquote.m_dMCJG1,
				!strcasecmp(type, "C") ? AMER_CALL : AMER_PUT);
		/* FIXME: avg price */
		if (fabs(quote->thyquote.m_dCJJJ) <= 0.000001)
			vol4 = NAN;
		else
			vol4 = impv_bi(savg,  strike, r, r, expiry, steps, quote->thyquote.m_dCJJJ,
				!strcasecmp(type, "C") ? AMER_CALL : AMER_PUT);
		if ((res = ALLOC(512))) {
			...
			snprintf(res, 512, "IMPV2,%s.%03d,%s|%.4f,%f,%.4f,%f,%.4f,%f,%f,%f,%.4f,%f",
				datestr,
				quote->m_nMSec,
				contract,
				last,
				...
				savg);
			out2rmp(res);
			snprintf(res, 512, "IMPV2,%d,%d,%s,%.4f,%f,%.4f,%f,%.4f,%f,%f,%f,%.4f,%f,"
				"%s,%s,%f,%f,%f,%d,0,%d",
				quote->thyquote.m_nTime,
				quote->m_nMSec,
				contract,
