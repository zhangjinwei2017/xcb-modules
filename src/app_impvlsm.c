...
/* FIXME */
static char *app = "impvlsm";
static char *desc = "Implied Volatility (LSM)";
static char *fmt = "IMPVLSM,timestamp,contract,lastprice,vol,"
...
static inline void load_config(void) {
	/* FIXME */
	if ((cfg = config_load("/etc/xcb/impvlsm.conf"))) {
				...
				while (var) {
					if (!strcasecmp(var->name, "rate")) {
						...
						xcb_log(XCB_LOG_WARNING, "Unknown variable '%s' in "
							"category '%s' of impvlsm.conf", var->name, cat);
					var = var->next;
				}
...
static int impvlsm_exec(void *data, void *data2) {
	...
	if (p && p != contract && p != contract + dstr_length(contract) - 1 &&
		...
		if (fabs(last) <= 0.000001)
			...
			vol = impv_lsm(slast, strike, r, r, expiry, ssteps, tsteps, last,
		...
		if (fabs(quote->thyquote.m_dMRJG1) <= 0.000001)
			...
			vol2 = impv_lsm(slast, strike, r, r, expiry, ssteps, tsteps, quote->thyquote.m_dMRJG1,
				!strcasecmp(type, "C") ? AMER_CALL : AMER_PUT);
		/* FIXME: ask price 1 */
		if (fabs(quote->thyquote.m_dMCJG1) <= 0.000001)
			...
			vol3 = impv_lsm(slast, strike, r, r, expiry, ssteps, tsteps, quote->thyquote.m_dMCJG1,
				!strcasecmp(type, "C") ? AMER_CALL : AMER_PUT);
		/* FIXME: avg price */
		if (fabs(quote->thyquote.m_dCJJJ) <= 0.000001)
			vol4 = NAN;
		else
			vol4 = impv_lsm(savg,  strike, r, r, expiry, ssteps, tsteps, quote->thyquote.m_dCJJJ,
				!strcasecmp(type, "C") ? AMER_CALL : AMER_PUT);
		if ((res = ALLOC(512))) {
			...
			snprintf(res, 512, "IMPVLSM,%s.%03d,%s|%.4f,%f,%.4f,%f,%.4f,%f,%f,%f,%.4f,%f",
				datestr,
				quote->m_nMSec,
				contract,
				last,
				...
				savg);
			out2rmp(res);
			snprintf(res, 512, "IMPVLSM,%d,%d,%s,%.4f,%f,%.4f,%f,%.4f,%f,%f,%f,%.4f,%f"
				"%s,%s,%f,%f,%f,%d,%d,%d",
				quote->thyquote.m_nTime,
				quote->m_nMSec,
				contract,
...
static int load_module(void) {
	...
	load_config();
	if (msgs_init(&impvlsm_msgs, "impvlsm_msgs", mod_info->self) == -1)
		return MODULE_LOAD_FAILURE;
	if (start_msgs(impvlsm_msgs) == -1)
		return MODULE_LOAD_FAILURE;
	if (msgs_hook(&default_msgs, impvlsm_exec, impvlsm_msgs) == -1)
		return MODULE_LOAD_FAILURE;
	return register_application(app, impvlsm_exec, desc, fmt, mod_info->self);
}

static int unload_module(void) {
	if (check_msgs(impvlsm_msgs) == -1)
		return MODULE_LOAD_FAILURE;
	msgs_unhook(&default_msgs, impvlsm_exec);
	stop_msgs(impvlsm_msgs);
	msgs_free(impvlsm_msgs);
	config_destroy(cfg);
	table_free(&expiries);
	table_free(&optns);
	table_free(&spots);
...
MODULE_INFO(load_module, unload_module, reload_module, "Implied Volatility (LSM) Application");
