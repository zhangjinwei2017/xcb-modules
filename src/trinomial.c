...
void tri_amer_call_greeks(double spot, double strike, double r, double d, double vol, double expiry, int steps,
	...
	*gamma = ((f12 - f11) / (spot * up - spot) - (f11 - f10) / (spot - spot * dn)) /
		(0.5 * (spot * up - spot * dn));
	*theta = (f11 - f00) / dt;
	*vega  = (tri_amer_call(spot, strike, r, d, 1.002 * vol, expiry, steps) - f00) / (0.002 * vol);
	*rho   = (tri_amer_call(spot, strike, 1.02 * r, 1.02 * d, vol, expiry, steps) - f00) / (0.02 * r);
	FREE(call_values);
	FREE(prices);
}
...
void tri_amer_put_greeks(double spot, double strike, double r, double d, double vol, double expiry, int steps,
	...
	*gamma = ((f12 - f11) / (spot * up - spot) - (f11 - f10) / (spot - spot * dn)) /
		(0.5 * (spot * up - spot * dn));
	*theta = (f11 - f00) / dt;
	*vega  = (tri_amer_put(spot, strike, r, d, 1.002 * vol, expiry, steps) - f00) / (0.002 * vol);
	*rho   = (tri_amer_put(spot, strike, 1.02 * r, 1.02 * d, vol, expiry, steps) - f00) / (0.02 * r);
	FREE(put_values);
	FREE(prices);
}
